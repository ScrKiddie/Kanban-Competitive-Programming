...
function getSidebarOpenState(defaultOpen: boolean) {
...
function SidebarProvider({
  ...
  const [_open, _setOpen] = React.useState(() => getSidebarOpenState(defaultOpen))
...
function SidebarRail({ className, ...props }: React.ComponentProps<"button">) {
  return (
    <button
      ...
      tabIndex={-1}
      className={cn(
        "pointer-events-none absolute inset-y-0 z-20 hidden w-4 -translate-x-1/2 cursor-default transition-all ease-linear group-data-[side=left]:-right-4 group-data-[side=right]:left-0 after:absolute after:inset-y-0 after:left-1/2 after:w-[2px] sm:flex",
...
const sidebarMenuButtonVariants = cva(
  "peer/menu-button flex w-full cursor-pointer items-center gap-2 overflow-hidden outline-2 outline-transparent border-2 border-transparent rounded-base p-2 text-left text-sm ring-ring transition-[width,height,padding,background-color,color,border-color] duration-150 ease-out md:hover:bg-main md:hover:text-main-foreground md:hover:border-border focus-visible:bg-main focus-visible:text-main-foreground focus-visible:border-border disabled:pointer-events-none disabled:opacity-50 group-has-data-[sidebar=menu-action]/menu-item:pr-8 aria-disabled:pointer-events-none aria-disabled:opacity-50 group-data-[collapsible=icon]:size-8! group-data-[collapsible=icon]:p-2! [&>span:last-child]:truncate [&>svg]:size-4 [&>svg]:shrink-0",
  {
    variants: {
      size: {
        default: "h-8 text-sm",
  ...
  return (
    <Comp
      ...
      className={cn(
        ...
        showOnHover && "opacity-0 transition-opacity group-hover/menu-item:opacity-100 group-focus-within/menu-item:opacity-100",
        "group-data-[collapsible=icon]:hidden",
        className,
      )}
      {...props}
